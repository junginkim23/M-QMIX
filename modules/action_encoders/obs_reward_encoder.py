...
class ObsRewardEncoder(nn.Module):
    ...
    def predict(self, obs, actions):
        ...
        inputs = torch.cat([obs_reshaped, other_actions], dim=-1)
        ...
        pred_avg_input = torch.cat([obs_latent_avg, action_latent_avg], dim=-1)
    ...
    def forward(self):
        actions = torch.Tensor(np.eye(self.n_actions)).to(self.args.device)
    ...
    def other_actions(self, actions):
        ...
        for i in range(self.n_agents):
            ...
            _other_actions = torch.cat(_other_actions, dim=-1)
            other_actions.append(_other_actions)

        other_actions = torch.stack(other_actions, dim=1).contiguous().view(-1, (self.n_agents - 1) * self.n_actions)
